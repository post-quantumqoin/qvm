...
lazy_static::lazy_static! {
    static ref BLS_ZERO_ADDR_BYTES: [u8; BLS_PUB_LEN] = {
        let bz_addr = Network::Mainnet.parse_address("q3yaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaby2smx7a");
...
impl Address {
    ...
    /// Generates new address using pqc pubkey.
    pub fn new_pqc(pubkey: &[u8]) -> Result<Self, Error> {
        // if pubkey.len() != SECP_PUB_LEN {
        //     return Err(Error::InvalidSECPLength(pubkey.len()));
        // }
        Ok(Self {
            payload: Payload::PQC(address_hash(pubkey)),
        })
    }
...
impl fmt::Display for Address {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self.payload() {
            ...
            Payload::PQC(data) | Payload::Secp256k1(data) | Payload::Actor(data) => {
...
pub(self) fn parse_address(addr: &str) -> Result<(Address, Network), Error> {
    ...
    let addr = match protocol {
        ...
        Protocol::PQC | Protocol::Secp256k1 | Protocol::Actor | Protocol::BLS => {
            ...
            if match protocol {
                Protocol::PQC | Protocol::Secp256k1 | Protocol::Actor => PAYLOAD_HASH_LEN,
