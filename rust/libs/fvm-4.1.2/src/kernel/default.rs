    ...
    C: CallManager,
{
    fn root(&mut self) -> Result<Cid> {
        ...
        let cid = self
            ...
            .state;
        println!("root state self.actor_id:{} cid:{}",self.actor_id,cid);
    ...
    C: CallManager,
{
    fn verify_signature(
        ...
        let t: GasTimer = self.call_manager.charge_gas(
            self.call_manager
                .price_list()
                .on_verify_signature(sig_type, plaintext.len()),
        )?;
        println!("DefaultKernel verify_signature");
        
        match sig_type {
            SignatureType::MultiPqc|SignatureType::Delegated=>{
                Err(syscall_error!(IllegalArgument; "delegated and multiPqc types that shouldn't be here").into())
            },
            SignatureType::BLS | SignatureType::Secp256k1=>{
                ...
                let signing_addr = match signer.payload() {
                    Payload::PQC(_) | Payload::BLS(_) | Payload::Secp256k1(_) => *signer,
                    // Not a key address.
                    _ => {
                ...
                t.record(catch_and_log_panic("verifying signature", || {
                    Ok(signature::verify(sig_type, signature, plaintext, raw_addr,&signing_addr).is_ok())
                }))
            },
            SignatureType::Falcon1024|SignatureType::Falcon512|SignatureType::Dilithium3|SignatureType::Dilithium5=>{
                t.record(catch_and_log_panic("verifying signature", || {
                    Ok(signature::verify(sig_type, signature, plaintext, raw_addr,&signer).is_ok())
                }))
            },
        }
    }
    ...
    fn recover_secp_public_key(
        ...
        println!("DefaultKernel recover_secp_public_key");
        t.record(
    ...
    C: CallManager,
{
    fn resolve_address(&self, address: &Address) -> Result<ActorID> {
        ...
        println!("DefaultKernel resolve_address address:{}",address.to_string());
        t.record(Ok(self
            .call_manager
    ...
    fn get_actor_code_cid(&self, id: ActorID) -> Result<Cid> {
        ...
        println!("DefaultKernel get_actor_code_cid id:{} state:{}",id,self
        .call_manager
        .get_actor(id)?
        .ok_or_else(|| syscall_error!(NotFound; "actor not found"))?
        .state);
    ...
    fn get_builtin_actor_type(&self, code_cid: &Cid) -> Result<u32> {
        ...
        let id = self
            ...
            .builtin_actors()
            .id_by_code(code_cid);
        println!("DefaultKernel get_builtin_actor_type code_cid:{} id:{}",code_cid,id);
        t.stop();
        Ok(id)
    }
    ...
    fn balance_of(&self, actor_id: ActorID) -> Result<TokenAmount> {
        ...
        println!("DefaultKernel balance_of id:{}",actor_id);
    ...
    fn lookup_delegated_address(&self, actor_id: ActorID) -> Result<Option<Address>> {
        ...
        println!("DefaultKernel lookup_delegated_address id:{}",actor_id);
        Ok(t.record(self.call_manager.get_actor(actor_id))?
            .ok_or_else(|| syscall_error!(NotFound; "actor not found"))?
            .delegated_address)
    }
